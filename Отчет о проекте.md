...
- Карусели подборок/избранного, режимы `Авто/Вручную`, скорость движения и темы `Прозрачнее/Контрастнее`.
- Усиленный responsive-режим: компактный mobile-блок `Сейчас играет`, свернутые найденные станции, улучшенные badge/счетчики и контрастные чипы.
...
- Lifetime text error log в админке: dashboard preview, отдельная вкладка журнала, поиск, фильтр по типам, filtered export и скачивание полного `.log`.
- Авто-ротация и архивирование старых логов с чтением активного файла и архивов как единой истории.
- IPTV seed-источники.
...
- Endpoint `POST /api/client-errors` для клиентских `runtime/promise/vue/http` ошибок с записью в lifetime log.
...
- Lifetime diagnostics log в `storage/logs/site-errors.log` с типами `server_exception`, `client_error`, `analytics_failure`, filtered export и archive rotation.
- Автоматическое переключение метода расчета интересов на time-based (`time_minutes`) при наличии heartbeat-данных.

Формулы, источники таблиц, алгоритмы fallback и ручная проверка зафиксированы в этом отчете, README и Swagger/OpenAPI.
...
- `PHP ^8.5` (Docker-образ на `PHP 8.5 FPM`)
- `Laravel 13`
- `Laravel Sanctum 4`
- `Laravel Reverb 1.10`
- `L5 Swagger 11`
...
- `app/Services/SiteErrorLogService`: lifetime text log сайта, preview/search/export/download и ротация архивов.
...
- Route-level rate limiting API:
  ...
  - public client error log: `throttle:30,1`,
...
- Подключить внешний агрегатор и алерты (Sentry + Loki/ELK) поверх уже встроенного lifetime error log.
...
- lifetime error log: server/client/analytics failure записи, admin preview/search/filter/export/download и archive rotation.
- встроенная документация по формулам, источникам данных, fallback-логике и командам ручной верификации в README, Swagger/OpenAPI и этом отчете.
...
- `docker compose run --rm test` — полный PHP feature/unit набор.
- `docker compose run --rm --no-deps --entrypoint npm node run test:js` — frontend helper/unit набор.
- `docker compose run --rm --no-deps frontend-build` — production build на Vite.
- `npm outdated` — контроль устаревших проектных пакетов.
- `npm audit` — контроль npm-уязвимостей.
- `composer audit` — контроль PHP security advisories; abandoned-зависимость `doctrine/annotations` удалена после миграции Swagger/OpenAPI на PHP attributes.
