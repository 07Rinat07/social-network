...
- Выгрузка аналитики дашборда в `Excel (.xls)` и `JSON`.
- IPTV seed-источники.
...
- `npm run test:js` (unit-тесты таймеров/сессионной радио-логики).
...
- `composer audit`: `0 advisories` (при этом `doctrine/annotations` остается `abandoned` транзитивной зависимостью через `l5-swagger`, без security advisory).
