...
6. Админ-дашборд использует heartbeat-агрегаты и переключает метод на `time_minutes` при наличии данных.
...
- `composer audit`: `0 advisories`; при этом `doctrine/annotations` остается `abandoned` транзитивной зависимостью через `l5-swagger`.
...
- Public/Site/Radio/IPTV/Chat/Admin Chat endpoints,
- Activity tracking: `POST /api/activity/heartbeat`,
- Admin analytics: `GET /api/admin/summary`, `GET /api/admin/dashboard`, `GET /api/admin/dashboard/export`.

Это обеспечивает согласованность API-контрактов для фронтенда, ручного тестирования и external QA.
