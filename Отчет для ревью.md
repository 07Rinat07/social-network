...
- `php artisan test tests/Feature/SwaggerDocumentationFeatureTest.php` (генерация Swagger/OpenAPI, наличие ключевых routes, включая client analytics/admin export, и доступность UI).
...
Это обеспечивает согласованность API-контрактов для фронтенда, ручного тестирования и external QA. Актуальная версия спецификации: `1.2.1`. Генерация спеки и доступность Swagger UI дополнительно проверяются `SwaggerDocumentationFeatureTest`.
