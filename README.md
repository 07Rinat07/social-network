...
- Upload-валидация пост-медиа опирается на серверный `MIME`/`Fileinfo`; клиентский `MIME` используется только как fallback для контейнеров вроде `mkv`, которые браузер может отправлять как `application/octet-stream`.
- Файлы, замаскированные под видео, но распознанные сервером как не-media (`application/pdf` и т.п.), отклоняются до сохранения.
...
### Видео не загружается в новый пост
- Поддерживаются `mp4`, `webm`, `mov`, `m4v`, `avi`, `mkv` и изображения `jpg/jpeg/png/webp/gif`.
- Лимит одного файла: `200 MB`.
- В личном кабинете показываются статус и текст ошибки по каждому файлу; если загрузка отклонена, причина видна под формой.
- Для `mkv` загрузка и скачивание поддерживаются, но встроенное воспроизведение в браузере зависит от кодеков конкретного файла.
- В production проверьте, что `Nginx`/reverse proxy разрешает тело запроса не меньше `200M`, иначе запрос будет отклонен до Laravel.
